...
@section('content')
    <div class="row">
            ...
            <div class="tablet">
                <div class="tablet__head">
                    <div class="tablet__head-label text-center">
                        <h3 class="tablet__head-titled">{{ __('Invoice summary') }}</h3>
                    </div>
                </div>
                            ...
                            <div class="tablet__item" style="padding: 0;">
                                <div class="tablet__item__info">
                                    ...
                                    <div class="tablet__item__description">
                                        <p class="invoice-info">{{ $invoice_line->quantity }} x
                                            {{ $invoice_line->price_converted }}</p>
                                ...
                                <div class="tablet__item__toolbar">
                                    <div class="dropdown dropdown-inline">
                                        @if ($invoice->canUpdateInvoice())
                                            <form action="{{ route('invoiceLine.destroy', $invoice_line->external_id) }}"
                                                method="post">
                                                @method('delete')
                                                {{ csrf_field() }}
                                                <p>
                                        ...
                                        @if ($invoice->canUpdateInvoice())
                                            <button type="submit"
                                                class="fa fa-btn fa-trash-o btn btn-clean trashcan-icon"></button>
                                            </p>
                                            </form>
                                        @endif
                    ...
                    <div class="row">
                        @foreach ($reminders as $reminder)
                            <div class="col-md-3" style="padding-bottom: 1em;">
                                ...
                                @if ($reminder->id == 1)
                                    <button type="button" id="vddf" class="btn btn-md btn-brand btn-full-width closebtn"
                                        <?php $titleText = !$invoice->is_vddf_date() ? __("Can't") : ''; ?> title="{{ $titleText }}" data-toggle="modal"
                                        data-target="#invoice-reminder-{{ $reminder->id }}"
                                        {{ !$invoice->is_vddf_date() ? 'disabled' : '' }}>
                                        {{ $reminder->name }} -
                                        {{ date(carbonDate(), strtotime($invoice->vddf_date())) }}
                                    </button>
                                @elseif ($reminder->id == 2)
                                    <button type="button" id="vddf" class="btn btn-md btn-brand btn-full-width closebtn"
                                        <?php $titleText = !$invoice->is_cdef_date() ? __("Can't") : ''; ?> title="{{ $titleText }}" data-toggle="modal"
                                        data-target="#invoice-reminder-{{ $reminder->id }}"
                                        {{ !$invoice->is_cdef_date() ? 'disabled' : '' }}>
                                        {{ $reminder->name }} -
                                        {{ date(carbonDate(), strtotime($invoice->cdef_date())) }}
                                    </button>
                                @elseif ($reminder->id == 3)
                                    <button type="button" id="vddf" class="btn btn-md btn-brand btn-full-width closebtn"
                                        <?php $titleText = !$invoice->is_cap_date() ? __("Can't") : ''; ?> title="{{ $titleText }}" data-toggle="modal"
                                        data-target="#invoice-reminder-{{ $reminder->id }}"
                                        {{ !$invoice->is_cap_date() ? 'disabled' : '' }}>
                                        {{ $reminder->name }} -
                                        {{ date(carbonDate(), strtotime($invoice->cap_date())) }}
                                    </button>
                                @elseif ($reminder->id == 4)
                                    <button type="button" id="vddf" class="btn btn-md btn-brand btn-full-width closebtn"
                                        <?php $titleText = !$invoice->is_rp_date() ? __("Can't") : ''; ?> title="{{ $titleText }}" data-toggle="modal"
                                        data-target="#invoice-reminder-{{ $reminder->id }}"
                                        {{ !$invoice->is_rp_date() ? 'disabled' : '' }}>
                                        {{ $reminder->name }} -
                ...
                <div class="tablet__body" style="padding-bottom: 3em;">
                    <p class="invoice-title">{{ $client->company_name }}
                        <a href="{{ route('clients.show', $client->external_id) }}"><i class="ion ion-ios-redo"
                                title="{{ __('Go to client') }}"
                                style="float: right;margin-right: 1em;color:#61788b;"></i></a>
                    </p>
                    ...
                    <div class="row">
                        ...
                        @if ($invoice->offer)
                            <div class="col-md-6">
                                ...
                                <p class="invoice-info-subtext">
                                    <button data-offer-external_id={{ $invoice->offer->external_id }}
                                        class="btn btn-link" style="padding: 0px;"
                                        id="view-original-offer">@lang('Offer')</button>
                                </p>
                            </div>
                        @endif
                        ...
                        {{-- <div class="col-md-6" style="padding-bottom: 1em;">
                            @if (Entrust::can('invoice-pay'))
                                <button type="button" id="update-payment"
                                    class="btn btn-md btn-brand btn-full-width closebtn" <?php $titleText = !$invoice->isSent() ? __("Can't pay an invoice with status draft. Send invoice first or force a new status") : ''; ?>
                                    title="{{ $titleText }}" {{ !$invoice->isSent() ? 'disabled ' : '' }}
                                    data-toggle="modal" data-target="#update-payment-modal">@lang('Register
                                    payment')</button>
                            @endif
                        </div> --}}
                        ...
                        <div class="col-md-6" style="padding-bottom: 1em;">
                            @if (Entrust::can('invoice-send'))
                                <button type="button" id="sendInvoice" class="btn btn-md btn-brand btn-full-width closebtn"
                                    value="add_time_modal" <?php $titleText = $invoice->isSent() ? __('Invoice already sent') : ''; ?> title="{{ $titleText }}"
                                    {{ $invoice->isSent() ? 'disabled ' : '' }} data-toggle="modal"
                                    data-target="#SendInvoiceModalConfirm">
                                    {{ __('Send invoice') }}
                                </button>
                            @endif
                        </div>
        ...
        <div class="col-md-12">
            @include('invoices._reminderList')
        </div>
    </div>
    <div class="modal fade" id="view-offer" tabindex="-1" role="dialog" aria-hidden="true" style="display:none;">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        @if ($apiconnected)
                            ...
                            <select name="invoiceContact" class="small-form-control bootstrap-select contacts-selectpicker"
                                id="user-search-select" data-live-search="true"
                                data-style="btn btn-md dropdown-toggle btn-light" data-container="body">
                                <option value="" style="color:lightslategrey"> @lang("Nothing selected")</option>
                                @foreach ($contacts as $contact)
                                    <option data-tokens="{{ $contact['name'] }}" value="{{ $contact['guid'] }}"
                                        {{ optional($primaryContact)['guid'] == $contact['guid'] ? 'selected' : '' }}>
                                        {{ $contact['name'] }}
                                    </option>
                                @endforeach
                            ...
                            <div id="sendMailBox">
                                <label for="sendMail" class="control-label" id="sendMailCheckboxLabel">@lang('Send mail
                                    with invoice to Customer?(Cheked = Yes):')</label>
                                <input type="checkbox" name="sendMail" id="sendMailCheckbox">
                                <p style="font-size:10px;">
                                    {{ __('The Mail will be send with your default settings and template from your billing integration.') }}
                                </p>
                            </div>
                        @endif
                        <div id="send-mail" style="display: none">
                            ...
                            @lang('Recipient')
                            <input type="text" class="form-control" name="recipientMail"
                                value="{{ $invoice->client->primaryContact->email }}">
                            @lang('Subject')
                            <input type="text" class="form-control" name="subject"
                                value="{{ __('Invoice from :company', ['company' => $companyName]) }}">

                            @lang('Message') (@lang("[link-to-pdf], will be replaced when invoice is send, with the actual
                            link to the PDF"))
    ...
    <div class="modal fade" id="update-payment-modal" tabindex="-1" role="dialog" aria-hidden="true"
        style="display:none;">
    ...
    @foreach ($reminders as $reminder)
        <div class="modal fade" id="invoice-reminder-{{ $reminder->id }}" tabindex="-1" role="dialog"
            aria-hidden="true" style="display:none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    @include('invoices._invoiceReminderModal', [
                        'reminder' => $reminder,
                    ])
                </div>
            </div>
